...
## Next Up
- **Session 2** (in progress): #9a Sonnet prompt improvements DONE + #9b intent helper backend DONE (D content preview, A content-based intents, B relevance scoring). Safety tags: `v4.1-sonnet-prompt-research-upgrade`, `v4.2-intent-helper-backend`. C (query generation) deferred. Remaining: test playbooks + intent flow, commit + push.
